...
describe Report do
  ...
  describe '#season_avr_arr' do
    it 'Should return an array with the arguments passed to the method' do
      expect(report.send(:season_avr_arr, 50, 20, 20, 0.45)).to eql([50, 20, 20, 0.45])
    end
    it 'Should not return an array with the arguments on different order' do
      expect(report.send(:season_avr_arr, 50, 20, 20, 0.45)).not_to eql([20, 50, 20, 0.45])
    end
  end
  describe '#last_ten_arr' do
    it 'Should return an array with the arguments passed to the method' do
      expect(report.send(:last_ten_arr, 50, 20, 20, 0.45)).to eql([50, 20, 20, 0.45])
    end
    it 'Should not return an array with the arguments on different order' do
      expect(report.send(:last_ten_arr, 50, 20, 20, 0.45)).not_to eql([20, 50, 20, 0.45])
    end
  end
